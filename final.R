...
get_outofsample_sharperatio_returns = function (M, data, sharperatio, strategy) {
  ...
  if (sharperatio == TRUE) { # computer the out-of-sample sharperatio
...
bm_comparison = function (benchmark) {
  ...
  #plot a basic ggplot
  bm= ggplot(melt(bm_mat), aes(x=Var1, y=value, col=Var2))+geom_line(alpha = 0.7)
...
# create a data frame of the excessive returns subset
...
get_outofsample_sharperatio_returns(120, data.new, TRUE, "mv")

#1.2) in-sample

# 1. alternative
get_insample_sharperatio_returns(data.new, get_weights_vector((get_means_vector(data.new)), cov(data.new)), TRUE)
...
# compute the unadjusted out-of-sample portfolio returns
unadjusted_returns_out= get_outofsample_sharperatio_returns(120, data.probe, FALSE, "mv")

# compute the out-of-sample sharpe ratio
mean(unadjusted_returns_out) - ((1/2)*(var(unadjusted_returns_out)))

#3.2) in-sample

# compute the absolute weights vector of the unadjusted returns: needed as a parameter for the following function
absolute_weights_unadj = get_weights_vector((get_means_vector(data.probe)), cov(data.probe))

# compute the unadjusted in-sample portfolio returns
unadjusted_returns_in = get_insample_sharperatio_returns(data.probe, absolute_weights_unadj, FALSE )

mean(unadjusted_returns_in) - (1/2)*(var(unadjusted_returns_in))
...
get_weights_dynamics(120, data.probe, 11, 1)

# based on adjusted portfolio returns
get_weights_dynamics(120, data.new, 11, 1)
...
get_sd_dynamics_or_devreturn(unadjusted_returns_in, 6, FALSE, dates_in)

# based on unadjusted out-of-sample portfolio returns
get_sd_dynamics_or_devreturn(unadjusted_returns_out, 6, FALSE, dates_out)
...
get_outofsample_sharperatio_returns(120, data.new, TRUE, "kz")

#1.2) in-sample

# 1. alternative
get_insample_sharperatio_returns(data.new, get_weights(data.new, observations, assets), TRUE)
...
# compute the unadjusted out-of-sample portfolio returns
unadjusted_returns_out_kz= get_outofsample_sharperatio_returns(120, data.probe, FALSE, "kz")

# compute the out-of-sample sharpe ratio
mean(unadjusted_returns_out_kz) - ((1/2)*(var(unadjusted_returns_out_kz)))

#3.2) in-sample

# compute the absolute weights vector of the unadjusted returns: needed as a parameter for the following function
absolute_weights_unadj_kz = get_weights(data.probe, observations, assets)

# compute the unadjusted in-sample portfolio returns
unadjusted_returns_in_kz = get_insample_sharperatio_returns(data.probe, absolute_weights_unadj_kz, FALSE )

mean(unadjusted_returns_in_kz) - (1/2)*(var(unadjusted_returns_in_kz))
...
# based on unadjusted portfolio returns
get_weights_dynamics(120, data.probe, 11, 0)

# based on adjusted portfolio returns
get_weights_dynamics(120, data.new, 11, 0)

#7) Plotting the dynamics of a portfolio's returns: the plot is based on a 6-month interval

# based on unadjusted in-sample portfolio returns
get_sd_dynamics_or_devreturn(unadjusted_returns_in_kz, 6, FALSE, dates_in_kz)

# based on unadjusted out-of-sample portfolio returns
get_sd_dynamics_or_devreturn(unadjusted_returns_out_kz, 6, FALSE, dates_out_kz)
